...
void draw() {
            ...
            if(sent==true){
              ...
              if(myPort.available() > 0 && modo == true) //Mostrar mensaje recibido modo master
                {
                  ...
                  if(mensajef == mensaje){                    //Si el mensaje que llego es el mismo que se envio entonces muestralo en pantalla
                  println("MENSAJE RECIBIDO: "+ mensajef);
                  println("MENSAJE RECIBIDO: "+ binary(mensajef));
                  mostrarmensaje(mensajef);
                  }
                }
              
              
                if(myPort.available() > 0 && modo ==false) //Mostrat mensaje recibido modo esclavo
                {
                  mensajeb = myPort.readBytes(); 
                  println("trama recibida");
                  println(binary(mensajeb[0]));
...
void mostrarmensaje(char msj)
{
   clear();
   textSize(30);
   text("Mensaje recibido: "+msj,0,30);
}
